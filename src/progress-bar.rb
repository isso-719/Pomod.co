def progress_bar_today
  unless UserSetting.find_by(user_id: session[:user]).nil?
    ...
    if current_user.pomodoros.nil?
      ...
      if current_user.pomodoros.find_by('created_at >= ?', Date.today).nil?
        @pomodoro = 0
      else
        @pomodoro = current_user.pomodoros.where('created_at >= ?', Date.today).sum(:time)
      end
...
def progress_bar_week
  unless UserSetting.find_by(user_id: session[:user]).nil?
    ...
    if current_user.pomodoros.nil?
      ...
      if current_user.pomodoros.find_by('created_at >= ?', 1.week.ago).nil?
        @pomodoro_week = 0
      else
        @pomodoro_week = current_user.pomodoros.where('created_at >= ?', 7.week.ago).sum(:time)
      end
